...
public:
    Elgamal( ) {
        d = BN_new(); x = BN_new(); y = BN_new(); // d = prime p; x = g (p's generator); y = g^x mod p
        priv = BN_new(); // priv = x (random value in Zp group)
        //Setup 
        BN_generate_prime_ex(d, LENGTH * 8, 1, NULL, NULL, NULL); // 1024?? (-> LENGTH 값을 128로 바꾸면 되나?)
    ...
    void Enc(U8* msg) {
        ...
        // TODO START   //////////////////////////////////////////////////////////////////////////////
        BN_rand_range(r, d); // r = random value in Zp group
        BN_Square_Multi(c0, x, r, d); // c0 = x^r mod d (= g^r mod p)
        BN_Square_Multi(c1, y, r, d); BN_mod_mul(c1, m, c1, d, ctx); // c1 = m*y^r mod d (= m*y^r mod p)
        // TODO END     //////////////////////////////////////////////////////////////////////////////
    ...
    void Dec(U8** cipher) {
        ...
        // TODO START   //////////////////////////////////////////////////////////////////////////////
        BN_Square_Multi(c0_exp_priv, c0, priv, d); // c0_exp_priv = c0^priv mod d (= c1^x mod p)
        ExtendedEuclidean.algorithm(d, c0_exp_priv); // gcd(p, c1^x) = x*p + y*(c1^x) (=> y is inverse)
        BN_copy(inv, ExtendedEuclidean.getY()); // inv = (c1^x)'s inverse
        // TODO END     //////////////////////////////////////////////////////////////////////////////
