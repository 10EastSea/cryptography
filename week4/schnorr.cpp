...
public:
    ...
    void Sign(U8* msg) {
        ...
        BIGNUM *c = BN_new(); BIGNUM *z = BN_new();
        ...
        BIGNUM *p_1 = BN_new(); BN_sub(p_1, p, BN_value_one());
        BN_mod_mul(z, x, c, p_1, ctx); BN_mod_add(z, r, z, p_1, ctx); // z = (r + xc) mod p-1
        BN_free(p_1);
        // TODO END     //////////////////////////////////////////////////////////////////////////////
    ...
    int Verify(U8** sig, U8* msg) {
        ...
        BN_Square_Multi(g_exp_c1, g, c1, p); // g^z
        BN_Square_Multi(y_exp_c0, y, c0, p); // y^c
        ExtendedEuclidean.algorithm(p, y_exp_c0);
        BN_copy(y_exp_c0_inv, ExtendedEuclidean.getY()); // (y^c)'s inverse
        BN_mod_mul(R_prime, g_exp_c1, y_exp_c0_inv, p, ctx); // R' = (g^z) * inv(y^c)

        U8 digest[SHA256_DIGEST_LENGTH] = {0};
        U8 *r_ = (U8*) calloc(BN_num_bytes(R_prime), sizeof(U8));
        SHA256_CTX hs = {0};
        SHA256_Init(&hs);
        BN_bn2bin(R_prime,r_);
        SHA256_Update(&hs, msg, strlen((char*)msg));
        SHA256_Update(&hs, r_, BN_num_bytes(R_prime));
        SHA256_Final(digest, &hs);
        BN_bin2bn(digest, SHA256_DIGEST_LENGTH, c_prime); // c' = H(m||R')
